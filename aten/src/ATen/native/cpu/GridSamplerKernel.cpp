...
struct ApplyGridSample<scalar_t, 2, GridSamplerInterpolation::Bilinear,
  ...
  compute_interp_params(const Vec& x, const Vec& y) const {
    ...
    auto nw_mask = at::vec::cast<scalar_t>(must_in_bound ? iVec(-1) : (w_mask & n_mask));
    auto ne_mask = at::vec::cast<scalar_t>(e_mask & n_mask);
    auto sw_mask = at::vec::cast<scalar_t>(w_mask & s_mask);
    auto se_mask = at::vec::cast<scalar_t>(e_mask & s_mask);
